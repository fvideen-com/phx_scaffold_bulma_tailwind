...
RUN apt-get update -y && apt-get install -y libstdc++6 openssl libncurses5 locales build-essential nodejs npm git \
  && apt-get clean && rm -f /var/lib/apt/lists/*_*
...
COPY --from=builder --chown=nobody:root /app/_build/prod/rel/phx_scaffold_bulma_tailwind ./
